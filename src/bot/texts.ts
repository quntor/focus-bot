...
export const T = {
  ...
  consent: (policyUrl: string | undefined) =>
    [
      ...
      '',
      'Для работы я обрабатываю твой Telegram ID, задачи, тексты и данные о сессиях. Голос передаётся сервису распознавания, но аудио не хранится. Тексты могут обрабатываться подключённой моделью.',
      'Нажимая «Согласен», ты даёшь согласие на обработку этих данных для работы бота, аналитики и обезличенной отчётности программы Сбер 500.',
      policyUrl ? `Политика обработки данных: ${policyUrl}` : 'Политика обработки данных пока недоступна — продолжить работу нельзя.',
      'Удалить рабочие данные и отозвать согласие можно командой /delete_me.',
    ].join('\n'),
