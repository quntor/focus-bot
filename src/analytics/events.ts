...
// Действие — то, что сделал человек. По ним считается DAU («воспользовался
// решением в течение суток») и внутренняя метрика «действия на DAU».
//
// Зачётное «обращение» конкурса — другое: вызов решением своих компонентов
// (модель, skill, инструменты), журнал component_calls, src/analytics/calls.ts.
// Сообщения бота не попадают ни туда, ни сюда: метрику нельзя поднимать
// рассылкой, не сделав продукт лучше.
